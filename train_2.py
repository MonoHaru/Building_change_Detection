...
from modules.datasets import SegDataset_2
...
if __name__ == '__main__':
    ...
    # Load config
    config_path = os.path.join(prj_dir, 'config', 'train_2.yaml')
    ...
    train_result_dir = os.path.join(prj_dir, 'results', 'train', 'DLV3P_efficient_b0_00_1')
    ...
    train_dataset = SegDataset_2(paths=train_img_paths,
                               input_size=[config['input_width'], config['input_height']],
                               scaler=get_image_scaler(config['scaler']),
                               logger=logger)
    val_dataset = SegDataset_2(paths=val_img_paths,
                             input_size=[config['input_width'], config['input_height']],
                             scaler=get_image_scaler(config['scaler']),
                             logger=logger)
    # Create data loader
    ...
    check_point = torch.load(r'D:\Maicon\results\train\DLV3P_efficient_b0_00\model.pt')
    ...
    logger.info(
        f"Load optimizer:{config['optimizer']['name']}, scheduler: {config['scheduler']['name']}, loss: {config['loss']['name']}, metric: {config['metrics']}")
    ...
    for epoch_id in range(config['n_epochs']):
        ...
        if early_stopper.stop:
            print(
                f"Epoch {epoch_id}/{config['n_epochs']}, Stopped counter {early_stopper.patience_counter}/{config['earlystopping_patience']}")
            logger.info(
                f"Epoch {epoch_id}/{config['n_epochs']}, Stopped counter {early_stopper.patience_counter}/{config['earlystopping_patience']}")
            break
