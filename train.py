...
if __name__ == '__main__':
    ...
    # Set device(GPU/CPU)
    # print(str(config['gpu_num']))
    os.environ['CUDA_VISIBLE_DEVICES'] = str(config['gpu_num'])
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    print(device)

    # Create train result directory and set logger
    train_result_dir = os.path.join(prj_dir, 'results', 'train', 'FPN_efficientb3_00_1')
    ...
    check_point = torch.load(r'D:\Maicon\results\train\FPN_efficientb3_00\model.pt')
    ...
    for epoch_id in range(config['n_epochs']):
        ...
        # print(row[config['earlystopping_target']])
